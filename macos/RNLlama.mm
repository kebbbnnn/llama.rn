...
// On macOS there is no simulator and every GPU is Metal-capable, so this is
// simpler than the iOS equivalent (which also crosses off simulator + Apple7).
MetalAvailability getMetalAvailability(bool skipGpuDevices) {
    ...
    // Fall back to Apple7 on Intel/AMD Macs that only report the Mac2 family.
