...
if ENV["RNLLAMA_DISABLE_METAL"] != "1" then
  base_compiler_flags += " -DLM_GGML_USE_METAL -DLM_GGML_METAL_EMBED_LIBRARY=1"
end

base_optimizer_flags = "-O3 -DNDEBUG -funroll-loops"

Pod::Spec.new do |s|
  s.name         = "llama-rn-macos"
  ...
  # react-native-macOS ships its own CocoaPods tree; this podspec is the
  # macOS counterpart to llama-rn.podspec.
  ...
  if ENV["RNLLAMA_BUILD_FROM_SOURCE"] == "1"
    s.source_files = "macos/**/*.{h,m,mm}", "cpp/**/*.{h,cpp,hpp,c,m,mm,s}"
    # Exclude standalone tooling sources (same exclusions as llama-rn.podspec).
  ...
  # Header-only JSON dependency needed by JSI when using the prebuilt framework
